...
\ifbook
\else
\ifoot{\hyperlink{contents}{{\textnormal{\contentsname}}}}
\cfoot{\hyperlink{index}{{\textnormal{\indexname}}}}
\fi



% Textos automáticos de LaTeX en castellano (Índice general, Capítulo, Figura...)
% y partición de palabras en castellano.
% es-noshorthands: evita que " < > pasen a ser caracteres activos, lo que
%                  rompería el código de los listings y las URL.
% es-tabla:        «Tabla» en lugar del «Cuadro» que pone babel por omisión.
% es-nosectiondot / es-noindentfirst: desactivan dos convenciones tipográficas
%                  castellanas (punto tras el número de apartado y sangrado del
%                  primer párrafo) para no alterar el aspecto del original.
%                  Quítalas si prefieres seguir la convención castellana.
\usepackage[spanish,es-noshorthands,es-tabla,es-nosectiondot,es-noindentfirst]{babel}
...
% listings trae sus propios rótulos y babel no los traduce
...
\ifbook
\else
\includepdf{chisel-cover-es.pdf}
\newpage
\thispagestyle{empty}
~
\newpage
...
\ \\
\vspace{1cm}
{\usekomafont{title}\mdseries\huge Diseño Digital con Chisel}
\ \\
\vspace{1cm}
{\usekomafont{title}\mdseries\Large Sexta Edición}
\cleardoublepage
\end{flushleft}
\newpage
...
Copyright \copyright{} 2016--2025 Martin Schoeberl
  ...
  \begin{tabular}{lp{.8\textwidth}}
    \raisebox{-12pt}{\includegraphics[height=18pt]{figures/cc_by_sa}} &
     Esta obra cuenta con una licencia Atribución-CompartirIgual 4.0 Internacional.
...
Correo electrónico: \url{martin@jopdesign.com}\\
Código fuente original disponible en \url{https://github.com/schoeberl/chisel-book}
\medskip
...
\phantomsection
% Ancla interna del enlace del pie de página: no es texto visible y tiene que
% coincidir con el \hyperlink{contents} del preámbulo. No se traduce.
...
Es un momento apasionante para dedicarse al mundo del diseño digital. Con el final del escalado de Dennard y 
la ralentización de la ley de Moore, quizá nunca haya habido una necesidad mayor de innovación en este campo. 
Las empresas de semiconductores siguen exprimiendo hasta la última gota de rendimiento que pueden, pero el coste 
de estas mejoras ha ido aumentando de forma drástica. Chisel reduce ese coste al mejorar la productividad. Si los 
diseñadores pueden construir más en menos tiempo, a la vez que amortizan el coste de la verificación mediante la 
reutilización, las empresas pueden gastar menos en ingeniería no recurrente (NRE, del inglés \textit{Non-Recurring 
Engineering}). Además, tanto los estudiantes como quienes trabajan pueden innovar más fácilmente por su cuenta.
...
Chisel se diferencia de la mayoría de los lenguajes en que está integrado dentro de otro lenguaje de programación, 
Scala. Básicamente, Chisel es una biblioteca de clases y funciones que representan las primitivas necesarias para 
expresar circuitos digitales síncronos. Un diseño en Chisel es en realidad un programa de Scala que \emph{genera} 
un circuito conforme se ejecuta. A muchos esto puede resultarles poco intuitivo: ``¿Por qué no hacer de Chisel un 
lenguaje independiente como VHDL o SystemVerilog?'' Mi respuesta a esta pregunta es la siguiente: el mundo del 
software ha visto una cantidad considerable de innovación en metodología de diseño durante las últimas dos décadas. 
En lugar de intentar adaptar esas técnicas a un nuevo lenguaje de hardware, podemos simplemente \emph{usar} un 
lenguaje de programación moderno y obtener esas ventajas gratis.
...
Una crítica que se le ha hecho a Chisel desde hace tiempo es que resulta ``difícil de aprender''. 
Gran parte de esa percepción se debe a la abundancia de diseños grandes y complejos, creados por expertos para 
resolver sus propias necesidades comerciales o de investigación. Cuando uno aprende un lenguaje popular como C++, 
no empieza leyendo el código fuente de GCC. Más bien comienza realizando algún curso, consultando libros de texto, 
u otros materiales didácticos pensados para los principiantes. Con \emph{Diseño Digital con Chisel}, Martin ha 
creado un recurso importante para cualquiera que desee aprender Chisel.
...
Martin es un docente con amplia experiencia, y eso se nota en la organización de este libro. Empezando por la instalación y 
las primitivas, va construyendo la comprensión del lector como un edificio, ladrillo a ladrillo. Los ejercicios que 
incluye son el mortero que consolida esa comprensión, asegurando que cada concepto fragüe en la mente del lector. 
El libro culmina con los \emph{generadores de hardware}, que son como el tejado que da sentido al resto de la 
estructura. Al final, el lector se queda con los conocimientos necesarios para construir un diseño sencillo pero 
útil: un procesador RISC.
...
En \emph{Diseño Digital con Chisel}, Martin ha sentado unas bases sólidas para un diseño digital productivo. 
Lo que construyas sobre ellas depende de ti.
...
Este libro es una introducción al diseño digital centrada en el uso del lenguaje de construcción de hardware Chisel. 
Chisel incorpora al diseño digital avances de la ingeniería del software, como los lenguajes orientados a objetos y 
...
Este libro va dirigido a diseñadores de hardware e ingenieros de software. Los diseñadores de hardware con 
conocimientos de Verilog o VHDL pueden aumentar su productividad con un lenguaje moderno para su próximo diseño ASIC o 
FPGA. Los ingenieros de software con conocimientos de programación orientada a objetos y funcional pueden aprovechar 
lo que ya saben para programar hardware, por ejemplo, aceleradores FPGA que se ejecutan en la nube.
...
El enfoque de este libro consiste en presentar componentes de hardware típicos, de tamaño pequeño a mediano, 
para explorar el diseño digital con Chisel.
...
No he utilizado ningún modelo grande de lenguaje (LLM) para escribir ni una sola frase. Todos los errores son míos, 
no una alucinación de un LLM. Utilizo Grammarly para la revisión gramatical y Copilot para escribir código.
...
\section*{Prefacio de la Segunda Edición}
...
Igual que Chisel permite un diseño de hardware ágil, también lo permiten el acceso abierto y la impresión bajo demanda, 
que hacen posible una publicación ágil de libros de texto. Menos de seis meses después de la primera edición de este 
libro puedo ofrecer una segunda edición mejorada y ampliada.
...
Además de correcciones menores, los cambios principales de la segunda edición son los siguientes. 
Se ha ampliado la sección dedicada a las pruebas. El capítulo de bloques de construcción secuenciales incluye 
más circuitos de ejemplo. Un nuevo capítulo sobre procesamiento de entradas explica la sincronización de entradas y 
muestra cómo diseñar un circuito antirrebotes y cómo filtrar una señal de entrada con ruido. El capítulo de diseños 
de ejemplo se ha ampliado para mostrar distintas implementaciones de una FIFO. Las variaciones de la FIFO muestran 
también cómo utilizar los parámetros de tipo y herencia en el diseño digital.

\section*{Prefacio de la Tercera Edición}
...
Chisel ha seguido avanzando durante el último año, así que ha llegado el momento de una nueva edición del libro de 
Chisel. Hemos actualizado todos los ejemplos a la última versión de Chisel (3.5.3) y a la versión recomendada de 
Scala (2.12.13).
...
Con Chisel 3.5, el entorno de pruebas \code{PeekPokeTester}, que forma parte del paquete \code{iotesters}, 
ha quedado obsoleto. Por ello, hemos adaptado la descripción de las pruebas al nuevo marco de trabajo 
\myref{https://github.com/ucb-bar/chiseltest}{ChiselTest}. Como todavía existen muchos diseños en Chisel que 
utilizan \code{PeekPokeTester}, hemos trasladado su descripción al apéndice.
...
Uno de los aspectos más interesantes del entorno Chisel/Scala/Java es que podemos aprovechar la infraestructura ya 
existente para distribuir bibliotecas de código abierto. Podemos publicar componentes de hardware en Maven 
con la misma facilidad que cualquier otra biblioteca Java de código abierto. Publicar en Maven significa que un 
componente de terceros se puede integrar en el flujo de compilación con una única referencia en la configuración de 
\code{build.sbt}. Este es el mismo procedimiento con el que incluyes la biblioteca de Chisel en tu diseño. Hemos añadido 
una sección sobre cómo publicar un diseño de Chisel en Maven Central.
...
Los \emph{generadores} de hardware se escriben en Scala. Por ello, hemos añadido una breve sección sobre Scala. 
Hemos ampliado el capítulo de generadores de hardware con una sección sobre el uso de la programación funcional 
para escribir generadores.
...
% Hans Jakob Damsgaard has contributed the description of how to use external components through 
% Chisel's \code{BlackBox} and how to use memories for clock domain crossing (multi-clock memories).

Hans Jakob Damsgaard ha contribuido con la descripción de cómo utilizar componentes externos mediante el 
\code{BlackBox} de Chisel y de cómo emplear memorias para el cruce de dominios de reloj (memorias multirreloj).

\section*{Prefacio de la Cuarta Edición}
...
Para la cuarta edición hemos pasado a la versión actual de Chisel, la 3.5.4. Hemos añadido un arbitrador (\textit{arbiter}), 
un codificador de prioridad (\textit{priority encoder}) y un comparador (\textit{comparator}) al capítulo sobre 
bloques de construcción combinacionales. Hemos ampliado el capítulo de generación de hardware con más ejemplos 
funcionales, entre ellos la construcción de un árbol de arbitraje equitativo a partir de sencillos circuitos 
de arbitraje 2 a 1. Hemos añadido un nuevo capítulo sobre interconexión, interfaces de bus y cómo conectar 
un dispositivo de E/S como dispositivo mapeado en memoria (\textit{memory-mapped device}). Hemos comenzado un nuevo 
capítulo sobre depuración, pruebas y verificación. Tenemos previsto ampliar en la próxima edición el capítulo dedicado 
a este tema tan importante. Hemos ampliado el capítulo del procesador con una introducción más asequible a los 
microprocesadores, que incluye una figura de la ruta de datos (\textit{datapath}).

\section*{Prefacio de la Quinta Edición}
...
Para la quinta edición hemos actualizado a la versión actual de Chisel, la 3.5.6, y a Scala 2.13. 
Para hacer sitio a temas más avanzados, he eliminado los dos apéndices sobre Chisel 2 y sobre PeekPokeTester. 
Todos los proyectos que se mantienen activamente han migrado por fin a Chisel 3, al menos con la capa de 
compatibilidad. PeekPokeTester quedó obsoleto con Chisel 3.5 y se recomienda pasar a ChiselTest. Si los necesitas, 
esos dos capítulos están disponibles en las versiones anteriores del libro de Chisel, accesibles en PDF en la 
\myref{https://www.imm.dtu.dk/~masca/chisel-book.html}{página web} de este libro.
...
La quinta edición no incluye cambios importantes; se trata de una versión de consolidación. Hemos realizado una revisión 
considerable del texto para mejorar la claridad de la redacción. Chisel ha incorporado pequeñas mejoras de comodidad 
(por ejemplo, \code{ChiselEnum}) que cubrimos en esta edición. Hemos ampliado el capítulo del procesador y 
actualizado los fragmentos de código de Leros a la versión actual de Leros.

\section*{Prefacio de la Sexta Edición}
...
en Scala, lo que significa que se ejecuta sobre la máquina virtual de Java y es independiente de la plataforma.
Después de la versión 3.x, la arquitectura del proceso de compilación de Chisel ha cambiado. Para reflejar ese cambio, 
el sistema de numeración de versiones se ha modificado para incrementar el número de versión mayor. La versión 4.0 se 
omite, y la siguiente versión de Chisel es la 5.0.
A partir de la versión 5.0, el backend del compilador se basa en \myref{https://circt.llvm.org/}{CIRCT}, un compilador 
de circuitos que forma parte del proyecto \myref{https://llvm.org/}{LLVM}. La principal diferencia para el usuario de 
Chisel es que CIRCT (y LLVM) están escritos en C++ y, por tanto, necesitan un ejecutable distinto para cada sistema 
operativo (arquitectura de procesador).
...
Hemos probado el libro con las versiones de Chisel 3.5.6, 3.6.1, 5.3.0 y 6.5.0. Chisel 3.6 es la última versión que 
utiliza el backend basado en Scala para generar los ficheros Verilog. Chisel 5 usa CIRCT y requiere instalar 
\code{firtool} manualmente. La última versión de Chisel 6 incluye los binarios de \code{firtool}. Las pruebas con 
ChiselTest siguen estando soportadas hasta Chisel 6. Por ello, recomendamos saltarse Chisel 5 y utilizar Chisel 6. 
Es posible que Chisel 6 cambie a una biblioteca distinta de simulación y pruebas. Hemos usado Scala 2.13 y lo hemos 
probado con Java 17 y Java 21. Chisel 3.5.6 no es compatible con Java 21; por eso recomendamos utilizar Chisel 3.6.1 
si se quiere trabajar con Chisel 3.

% \todo{Some of this might go into getting started.}

% \todo{We should also show more Leros code.}

\todo{Parte de esto podría ir en el capítulo de introducción.}
\todo{También deberíamos mostrar más código de Leros.}
...
Hemos ampliado el capítulo de pruebas con una descripción de cómo acceder a las señales internas y cómo utilizar 
aserciones (\textit{assertions}). Además, hemos empezado a explorar la verificación formal en Chisel.
...
La segmentación de los circuitos digitales en \emph{pipelines} para aumentar el rendimiento es un aspecto clave 
del diseño digital y de la arquitectura de computadores. Hemos añadido un capítulo sobre \emph{pipelining} en el 
que utilizamos como ejemplo práctico un \emph{pipeline} RISC-V de tres etapas.
...
Somos conscientes de que Chisel sigue siendo un lenguaje minoritario en el diseño de hardware y de que es necesario 
ser capaz, al menos, de leer VHDL o Verilog. Sin embargo, creemos que, con un buen conocimiento del diseño digital, 
cambiar a otro lenguaje de descripción de hardware debería ser cuestión de días. Para facilitar esa transición, 
hemos añadido un apéndice en el que se muestran y comparan las construcciones básicas en Chisel, VHDL y Verilog.
...
Este libro se ha escrito sin ninguna ayuda de la inteligencia artificial ni de modelos grandes de lenguaje para 
redactar o reescribir el texto. Hemos utilizado Grammarly\footnote{\url{https://app.grammarly.com/}} para la 
revisión gramatical y la mejora del estilo de redacción.
...
Este libro se ha traducido al chino, al japonés y al vietnamita. Todas las traducciones están disponibles como PDF 
gratuito en la \myref{https://www.imm.dtu.dk/~masca/chisel-book.html}{página web} del libro. Quiero dar las gracias 
a Yuda Wang, Qiwei Sun y Yun Chen por la traducción al chino; a Seiji Munetoh, Masatoshi Tanabata y Takaaki Hagino 
por la traducción al japonés; y a VieLe Duc Hung por la traducción al vietnamita. Si te interesa traducir este libro 
a otro idioma, hazlo sin problema y publícalo bajo la misma licencia. Ponte en contacto conmigo, para que 
pueda incluir un enlace a tu traducción.
...
Quiero dar las gracias a todas las personas que han trabajado en Chisel por crear un lenguaje de construcción de hardware tan 
estupendo. Es un auténtico placer utilizar Chisel y, por ello, merece que se escriba un libro sobre él. Estoy agradecido a toda la 
comunidad de Chisel, que es tan acogedora y amable y nunca se cansa de responder preguntas sobre Chisel.
...
También quiero dar las gracias a mis estudiantes de los últimos años en un curso avanzado de arquitectura de computadores, 
en el que la mayoría se decidió por Chisel para el proyecto final. Gracias por salir de vuestra zona de confort y emprender el 
viaje de aprender y usar un lenguaje de descripción de hardware puntero. Muchas de vuestras preguntas han ayudado a dar forma a 
este libro.
...
Ha sido un placer utilizar Chisel para impartir electrónica digital en la Universidad Técnica de Dinamarca desde 2020. 
Sé que supone un reto aprender Chisel y Java a la vez en el segundo semestre. Gracias a todos los estudiantes de este curso, 
que tuvieron la mente abierta para adoptar un lenguaje de programación moderno para la descripción de hardware.
...
Para la tercera edición, quiero reconocer el trabajo de Hans Jakob Damsgaard (\myref{https://github.com/hansemandse}{@hansemandse}), 
que reescribió todo el código de pruebas de este libro para ChiselTest, añadió ChiselTest al capítulo de pruebas e incorporó la 
descripción de las cajas negras (\emph{black boxes}) y un ejemplo de memoria multirreloj (\emph{multi-clock memory}).
...
Este libro es una introducción al diseño de sistemas digitales mediante un moderno lenguaje de construcción de hardware 
(\emph{hardware construction language}), \myref{https://www.chisel-lang.org/}{Chisel}~\cite{chisel:dac2012}.
En este libro nos centramos en un nivel de abstracción más alto de lo habitual en los libros de diseño digital, para que puedas 
construir sistemas digitales más complejos e interconectados en menos tiempo.
...
Este libro y Chisel se dirigen a dos grupos de desarrolladores:
(1) los diseñadores de hardware y (2) los programadores de software.
Los diseñadores de hardware que dominan VHDL o Verilog y utilizan otros lenguajes, como Python,
Java o Tcl, para generar hardware pueden pasarse a un único lenguaje de construcción de hardware
en el que la generación de hardware forma parte del propio lenguaje.
Los programadores de software pueden empezar a interesarse por el diseño de hardware,
por ejemplo, porque los futuros chips de Intel incluirán hardware programable para acelerar los programas.
Chisel es una opción perfectamente válida como primer lenguaje de descripción de hardware (\emph{hardware description language}).
...
Chisel traslada al ámbito del diseño digital avances de la ingeniería del software, como la programación orientada a objetos y 
la programación funcional. Chisel no solo permite describir el hardware en el nivel de transferencia de registros 
(\emph{register-transfer level}), sino que también permite escribir \emph{generadores} de hardware.
...
Hoy en día el hardware se describe habitualmente con un lenguaje de descripción de hardware.
La época de dibujar los componentes hardware, incluso con herramientas CAD, ya
pasó. Algunos esquemas de alto nivel pueden ofrecer una visión general del sistema, pero no
pretenden describirlo.
Los dos lenguajes de descripción de hardware más extendidos son Verilog y VHDL.
Ambos son lenguajes antiguos, contienen muchos elementos heredados y tienen una frontera cambiante sobre qué
construcciones del lenguaje son sintetizables en hardware.
No me malinterpretes: VHDL y Verilog son perfectamente capaces de describir un bloque de hardware
que puede sintetizarse en un
\myref{https://en.wikipedia.org/wiki/Application-specific_integrated_circuit}{ASIC}.
En el diseño de hardware con Chisel, Verilog hace de lenguaje intermedio
para las pruebas y la síntesis.
...
Este libro no es una introducción general al diseño digital ni a sus fundamentos.
Para una introducción a los conceptos básicos del diseño digital, como la construcción de una puerta lógica a partir de
transistores CMOS, consulta otros libros sobre diseño digital.
En cambio, este libro pretende enseñar diseño digital en un nivel de abstracción que se corresponde con
la práctica actual para describir ASIC o diseños destinados a
\myref{https://en.wikipedia.org/wiki/Field-programmable_gate_array}{FPGA}.\footnote{Como el autor está más familiarizado con las FPGA
que con los ASIC como tecnología de destino, algunas de las optimizaciones de diseño que se muestran en este libro
están orientadas a la tecnología FPGA.}
Como requisitos previos para este libro, se asumen conocimientos básicos del
\myref{https://en.wikipedia.org/wiki/Boolean_algebra}{álgebra de Boole} y del
\myref{https://en.wikipedia.org/wiki/Binary_number}{sistema numérico binario}.
Además, se presupone algo de experiencia de programación en cualquier lenguaje de programación.
No se necesita conocer Verilog ni VHDL.
Chisel puede ser tu primer lenguaje de programación para describir hardware digital.
Como el proceso de compilación de los ejemplos se basa en \code{sbt} y \code{make},
resultará útil un conocimiento básico de la interfaz de línea de comandos (CLI, también llamada terminal o
consola de Unix).
...
Chisel no es en sí un lenguaje grande. Las estructuras básicas caben en
\myref{https://github.com/freechipsproject/chisel-cheatsheet/releases/latest/download/chisel_cheatsheet.pdf}{una sola página}
y pueden aprenderse en unos pocos días.
Por eso, este libro tampoco es muy extenso.
Chisel es sin duda más pequeño que VHDL y Verilog, que arrastran muchos elementos heredados.
La potencia de Chisel proviene de su integración en
\myref{https://www.scala-lang.org/}{Scala}, que de por sí es un lenguaje expresivo.
Chisel hereda de Scala esa cualidad de ser ``un lenguaje que crece contigo''~\cite{Scala}.
Sin embargo, Scala no es el tema principal de este libro.
Incluimos una breve sección sobre Scala dirigida a los diseñadores de hardware.
El libro de texto de Odersky et al.~\cite{Scala} ofrece una introducción general
a Scala.
Este libro es un tutorial de diseño digital y del lenguaje Chisel; no es
un manual de referencia de Chisel ni un libro sobre diseño completo de chips.
...
Todos los ejemplos de código de este libro proceden de programas completos
compilados y probados mediante integración continua en GitHub.
Por lo tanto, el código no debería contener
ningún error sintáctico. Los ejemplos de código están disponibles en el
\myref{https://github.com/schoeberl/chisel-book}{repositorio de GitHub}
de este libro.
Además de mostrar código Chisel, hemos procurado mostrar diseños útiles y
principios de un buen estilo de descripción de hardware.
...
Este libro está optimizado para leerse en una tablet (por ejemplo, un iPad) o en un portátil.
Incluimos enlaces para ampliar información dentro del propio texto, principalmente a
artículos de \myref{https://en.wikipedia.org/}{Wikipedia}.

\section{Instalación de Chisel y de las herramientas para FPGA}
...
Chisel es una biblioteca de Scala, y la forma más sencilla de instalar Chisel y Scala es
con \code{sbt}, la herramienta de compilación de Scala. El propio Scala, a su vez, depende de la instalación
de \myref{https://www.oracle.com/technetwork/java/javase/downloads/jdk8-downloads-2133151.html}{Java JDK 8}
(o de una versión posterior hasta Java 21).
Como Oracle ha cambiado la licencia de Java, puede resultar más sencillo
instalar OpenJDK desde \myref{https://adoptopenjdk.net/}{AdoptOpenJDK}
o usar \myref{https://sdkman.io/}{SDKMAN} para gestionar tu instalación de Java.
...
Puedes encontrar instrucciones de instalación más detalladas en
\myref{https://github.com/schoeberl/chisel-lab/blob/master/Setup.md}{Setup.md},
dentro de \myref{https://github.com/schoeberl/chisel-lab}{chisel-lab}.
La \myref{https://github.com/schoeberl/chisel-lab/tree/master/lab1}{primera práctica}
explica cómo abrir un proyecto Chisel ya existente en IntelliJ.
...
Instala Java OpenJDK 8 (hasta la 21) desde \myref{https://adoptopenjdk.net/}{AdoptOpenJDK}.
En Mac OS X, con el gestor de paquetes \myref{https://brew.sh/}{Homebrew},
\code{sbt} y git se pueden instalar con:
...
Instala \myref{http://gtkwave.sourceforge.net/}{GTKWave} e
\myref{https://www.jetbrains.com/idea/download/}{IntelliJ} (la edición \emph{community}).
Al importar un proyecto, selecciona el JDK que has instalado antes.
...
En Ubuntu, que está basado en Debian, los programas se instalan normalmente a partir de un
archivo Debian (.deb). Sin embargo, en el momento de escribir estas líneas, \code{sbt} no
está disponible como paquete listo para instalar. Por eso, el proceso de instalación
es algo más laborioso.
Sigue las instrucciones de \myref{https://www.scala-sbt.org/download.html}{sbt download}
...
Instala Java OpenJDK (la 8 o hasta la 21) desde \myref{https://adoptopenjdk.net/}{AdoptOpenJDK}.
Chisel y Scala también se pueden instalar y utilizar en Windows.
Instala \myref{http://gtkwave.sourceforge.net/}{GTKWave} e
\myref{https://www.jetbrains.com/idea/download/}{IntelliJ} (la edición \emph{community}).
Al importar un proyecto, selecciona el JDK que has instalado antes.
\code{sbt} se puede instalar con un instalador de Windows; véase
\myref{https://www.scala-sbt.org/1.x/docs/Installing-sbt-on-Windows.html}{Installing sbt on Windows}.
Instala un \myref{https://git-scm.com/download/win}{cliente de git}.

\subsection{Herramientas para FPGA}
...
Para diseñar hardware para una FPGA necesitas una herramienta de síntesis. Los dos grandes
fabricantes de FPGA, Altera\footnote{Una compañía de Intel}
y Xilinx,\footnote{Una compañía de AMD} ofrecen versiones gratuitas de
sus herramientas que cubren las FPGA pequeñas y medianas. Esas FPGA
medianas son lo bastante grandes como para construir un procesador multinúcleo de tipo RISC.
Intel proporciona \myref{https://www.intel.com/content/www/us/en/products/details/fpga/development-tools/quartus-prime/resource.html}{Quartus Prime Lite Edition} y AMD,
\myref{https://www.xilinx.com/products/design-tools/vivado/vivado-webpack.html}{Vivado Design Suite, WebPACK Edition}.
Ambas herramientas están disponibles para Windows y Linux, pero no para macOS.

% With \myref{https://f4pga.org/}{F4PGA} it is now possible to use a fully open-source synthesis tool
% for selected FPGAs.

Con \myref{https://f4pga.org/}{F4PGA} ya es posible utilizar una herramienta de síntesis totalmente de código abierto
para determinados modelos de FPGA.
...
Todo libro sobre un lenguaje de programación debe empezar con un ejemplo mínimo,
llamado el ejemplo ``Hola Mundo'' (\emph{Hello World}). El siguiente código es una primera aproximación:

\shortlist{code/hello_scala.txt}

% \noindent Compiling and executing this short program with \code{sbt}

\noindent Al compilar y ejecutar este pequeño programa con \code{sbt}:
...
\noindent Se obtiene la salida esperada de un programa Hello World:
...
\noindent Sin embargo, ¿esto es Chisel? ¿Es esto hardware generado para imprimir una cadena de texto?
No, esto es simplemente código Scala y no un programa Hello World representativo de un diseño hardware.

\section{Chisel Hola Mundo}
...
¿Cuál es entonces el equivalente de un programa Hello World en un diseño hardware?
¿El diseño mínimo útil y visible? Un LED que parpadea es la versión hardware (o incluso
de software embebido) del Hello World. Si un LED parpadea, ¡ya estamos listos para
resolver problemas mayores!

\verylonglist{code/hello.txt}{Un Hello World hardware en Chisel}{lst:chisel:hello}
...
El código~\ref{lst:chisel:hello} muestra un LED parpadeante descrito en Chisel.
No es importante que entiendas los detalles de este ejemplo de código.
Los veremos en los capítulos siguientes. Fíjate simplemente en que el circuito
suele ir gobernado por un reloj de alta frecuencia, por ejemplo de 50~MHz, y necesitamos un contador
para obtener una temporización en el rango de los hercios y conseguir así un parpadeo visible. En el
ejemplo anterior contamos desde 0 hasta 25000000-1, y entonces conmutamos la señal de parpadeo
(\code{blkReg := ~blkReg}) y reiniciamos el contador (\code{cntReg := 0.U}).
Ese hardware hace parpadear el LED a 1~Hz.

\section{Un IDE para Chisel}
...
Este libro no presupone nada sobre tu entorno de programación o el editor que utilices.
Aprender lo básico debería resultar sencillo con solo usar \code{sbt} en la línea de comandos
y un editor de tu elección. Siguiendo la tradición de otros libros, todos los comandos que
debas teclear en un intérprete de órdenes/terminal/CLI/Consola (\emph{shell/terminal/CLI/Console}) van
precedidas por el carácter \code{\$}, que no debes teclear. Como ejemplo, aquí está el comando
\code{ls} de Unix, que proporciona un listado de los ficheros de la carpeta actual:
...
Dicho esto, un entorno de desarrollo integrado (IDE, \emph{integrated development
environment}), donde un compilador se ejecuta en segundo plano, puede acelerar la
escritura de código. Como Chisel es una biblioteca de Scala, todos los IDE
que soportan Scala son también buenos IDE para Chisel.
Es posible en
\myref{https://www.jetbrains.com/help/idea/discover-intellij-idea-for-scala.html}{IntelliJ},
\myref{https://code.visualstudio.com/}{Visual Studio Code} y
 \myref{https://www.eclipse.org/}{Eclipse}
generar un proyecto a partir de la configuración del proyecto sbt en \code{build.sbt}.
...
En IntelliJ necesitas instalar el plugin de Scala. Después puedes crear un nuevo proyecto a partir de archivos fuentes 
existentes con: \emph{File - New - Project from Existing Sources...} y seleccionar entonces el fichero
\code{build.sbt} del proyecto.
...
e importar ese proyecto en Eclipse.\footnote{Esta función necesita el plugin de Eclipse para sbt.}
...
\myref{https://code.visualstudio.com/}{Visual Studio Code} es otra opción como IDE para Chisel.
La extensión \myref{https://marketplace.visualstudio.com/items?itemName=scalameta.metals}{Scala Metals}
proporciona soporte para Scala.
En la barra de la izquierda selecciona \emph{Extensions}, busca \emph{Metals} e instala \emph{Scala (Metals)}.
Para importar un proyecto basado en \code{sbt}, abre la carpeta con \emph{File - Open}.
...
\section{Acceso al código fuente y al libro electrónico}
...
Este libro es de código abierto y está alojado en GitHub: \myref{https://github.com/schoeberl/chisel-book}{schoeberl/chisel-book}.
Todos los ejemplos de código Chisel que se muestran en este libro están incluidos en el repositorio.
Todo el código mostrado en el libro ha pasado por el compilador y, por tanto, no debería contener errores de sintaxis.
Además, la mayoría de los ejemplos incluyen también un banco de pruebas (\emph{test bench}).
El código se extrae automáticamente de ese código fuente.
\index{Chisel!Ejemplos}
Recopilamos ejemplos de Chisel más extensos en el repositorio complementario
\myref{https://github.com/schoeberl/chisel-examples}{chisel-examples}
y en \myref{https://github.com/freechipsproject/ip-contributions}{ip-contributions}.
...
Si encuentras algún error o errata en el libro, una \emph{pull request} en GitHub es la forma más cómoda de incorporar tu mejora.
También puedes aportar comentarios o sugerencias de mejora abriendo un \emph{issue} en GitHub
o enviando un simple correo electrónico a la vieja usanza.
...
El repositorio del libro también contiene
\myref{https://github.com/schoeberl/chisel-book/tree/master/slides}{transparencias en Latex}
que utilizo para un curso de 13 semanas sobre
\myref{http://www2.imm.dtu.dk/courses/02139/}{Electrónica Digital}\footnote{La página del curso
contiene las versiones en PDF de las transparencias} en la Universidad Técnica de Dinamarca.
Ese curso incluye además \myref{https://github.com/schoeberl/chisel-lab}{ejercicios de laboratorio}
en Chisel.
Si enseñas diseño digital con Chisel, siéntete libre de adaptar las transparencias y los ejercicios
de laboratorio a tus necesidades. Todo el material es de código abierto. Para compilar el libro y las
transparencias necesitas una versión reciente de Latex y las herramientas necesarias para Chisel
(\code{sbt} y una instalación de un JDK de Java). Todo el código se compila, se prueba, se extrae, y el
Latex se compila con un simple:
...
Este libro está disponible gratuitamente como libro electrónico en PDF y en versión impresa clásica en
\myref{https://www.amazon.com/dp/168933603X/}{Amazon}.
La versión electrónica incluye enlaces a recursos adicionales
y a entradas de \myref{https://www.wikipedia.org/}{Wikipedia}.
Usamos entradas de Wikipedia para información de contexto (por ejemplo, el sistema numérico binario)
que no encaja directamente en este libro.
Hemos optimizado el formato del libro electrónico para su lectura en una tablet, como un iPad.
...
\item \myref{http://www.cambridge.org/es/academic/subjects/engineering/circuits-and-systems/digital-design-systems-approach}{Digital Design: A Systems Approach}, de William J. Dally y R. Curtis Harting,
es un libro de texto sobre diseño digital. Está disponible en dos versiones: usando Verilog o VHDL como lenguaje de descripción de hardware.
\end{itemize}
...
\item La página web de \myref{https://www.chisel-lang.org/}{Chisel} es el punto de partida oficial para
descargar y aprender Chisel.
...
\item El sitio web del curso \myref{http://www2.imm.dtu.dk/courses/02139/}{Digital Electronics 2}
de la Universidad Técnica de Dinamarca contiene las transparencias de un curso de 13 semanas basado en Chisel.
El código fuente de las \myref{https://github.com/schoeberl/chisel-book/tree/master/slides}{transparencias}
está disponible como parte del código fuente de este libro. Siéntete libre de adaptarlas a tus necesidades docentes.
...
\item El repositorio de GitHub \myref{https://github.com/schoeberl/chisel-lab}{schoeberl/chisel-lab}
contiene ejercicios de Chisel para el curso \myref{http://www2.imm.dtu.dk/courses/02139/}{Digital Electronics 2}.
Los ejercicios encajan también bien para un autoaprendizaje con este libro.
...
\item El \myref{https://github.com/schoeberl/chisel-empty}{proyecto Chisel vacío} es un buen punto de
partida, con un hardware mínimo (un sumador) y una prueba. Ese proyecto es una plantilla de GitHub sobre
la que puedes basar tu propio repositorio de GitHub.
...
\item La \myref{https://github.com/freechipsproject/chisel-cheatsheet/releases/latest/download/chisel_cheatsheet.pdf}{chuleta de Chisel3} (\emph{Chisel3 Cheat Sheet}) resume los principales elementos de Chisel en una sola página.
...
\item El curso \myref{https://classes.soe.ucsc.edu/cse228a/Winter24/}{Agile Hardware Design} de Scott Beamer
contiene ejemplos avanzados de Chisel. Las \myref{https://github.com/agile-hw/lectures}{lecciones}
incluyen ejemplos de código ejecutables y están disponibles como cuadernos de Jupyter.

% \item \myref{https://github.com/ucb-bar/chiseltest}{ChiselTest} is in
% its own repository.
\item \myref{https://github.com/ucb-bar/chiseltest}{ChiselTest} está en
...
\item El \myref{https://github.com/freechipsproject/chisel-bootcamp}{Generator Bootcamp} es
un curso de Chisel centrado en los generadores de hardware, en forma de cuaderno de \myref{https://jupyter.org/}{Jupyter}
...
\item El \myref{https://github.com/ucb-bar/chisel-tutorial}{Chisel Tutorial} proporciona un proyecto ya
configurado con pequeños ejercicios, con \emph{testers} y soluciones. No obstante, está algo desactualizado.

% \item A \myref{https://github.com/ccelio/chisel-style-guide}{Chisel Style Guide} by Christopher Celio.
\item Una \myref{https://github.com/ccelio/chisel-style-guide}{guía de estilo de Chisel} de Christopher Celio.
...
Cada capítulo termina con unos ejercicios prácticos. Para el ejercicio de la introducción, usaremos una
placa FPGA para hacer parpadear un \myref{https://en.wikipedia.org/wiki/Light-emitting_diode}{LED}.\footnote{Si en
este momento no dispones de una placa FPGA, sigue leyendo, ya que al final del ejercicio te mostraremos una
versión en simulación.}
Como primer paso, clona (o haz un \emph{fork}) del repositorio
\myref{https://github.com/schoeberl/chisel-examples}{chisel-examples}
de GitHub.
El ejemplo del Hola Mundo está en la carpeta \code{hello-world}, configurada como
un proyecto mínimo. Puedes explorar el código Chisel del LED parpadeante
en \code{src/main/scala/Hello.scala}.
Compila el LED parpadeante con los siguientes pasos:
...
Tras una descarga inicial de los componentes de Chisel, esto producirá el fichero Verilog \code{Hello.v}.
Explora ese fichero Verilog. Verás que contiene dos entradas, \code{clock} y \code{reset},
y una salida, \code{io\_led}. Cuando compares este fichero Verilog con el módulo de Chisel,
notarás que el módulo de Chisel no contiene \code{clock} ni \code{reset}.
Esas señales se generan implícitamente y, en la mayoría de los diseños, resulta cómodo no tener que
lidiar con estos detalles de bajo nivel. Chisel proporciona componentes de registro, y estos
se conectan automáticamente a \code{clock} y \code{reset} (si es necesario).
...
El siguiente paso es preparar un proyecto de FPGA para la herramienta de síntesis, asignar los pines,
compilar\footnote{El proceso real es más elaborado, con los siguientes pasos: sintetizar la lógica,
realizar el emplazado y rutado (\emph{place and route}), realizar el análisis temporal y generar un fichero de
configuración (\emph{bitfile}).
No obstante, para el propósito de este ejemplo introductorio simplemente lo llamamos ``compilar''
tu código.} el código Verilog y configurar la FPGA con el \emph{bitfile} resultante.
No podemos dar más detalles para estos pasos. Si tienes curiosidad, consulta el manual de
tu herramienta Intel Quartus o AMD Vivado.
De todas formas, el repositorio de ejemplos contiene algunos proyectos de Quartus listos
para usar en la carpeta \code{quartus} para varias placas FPGA populares (por ejemplo, la DE2-115).
Si el repositorio incluye soporte para tu placa, arranca Quartus, abre el proyecto,
compílalo pulsando el botón \emph{Play} y configura la placa FPGA
con el botón \emph{Programmer}; entonces uno de los LED debería parpadear.

% {\bf Congratulations! You managed to get your first design in Chisel running in an FPGA!}
\textbf{¡Enhorabuena! ¡Has conseguido poner en marcha tu primer diseño en Chisel en una FPGA!}

% If the LED is not blinking, check the status of reset. On the DE2-115 configuration,
% the reset input is connected to SW0.
Si el LED no parpadea, comprueba el estado de la señal de \emph{reset}. En la configuración de la DE2-115,
la entrada de \emph{reset} está conectada al SW0.
...
Ahora cambia la frecuencia de parpadeo a un valor mayor o menor y
vuelve a ejecutar el proceso de compilación. Las frecuencias de parpadeo, y también los patrones de parpadeo,
transmiten distintas ``emociones''. Por ejemplo, un LED que parpadea despacio indica que
todo está bien, mientras que un LED que parpadea rápido indica un estado de alarma.
Explora qué frecuencias expresan mejor esas dos emociones distintas.
...
Como extensión más compleja del ejercicio, genera el siguiente patrón de parpadeo:
el LED debe estar encendido durante 200~ms cada segundo. Para este patrón, quizá te convenga
desacoplar el cambio del parpadeo del LED del instante en que se reinicia el contador.
Necesitarás una segunda constante en la que cambies el estado del
registro \code{blkReg}. ¿Qué tipo de emoción produce este patrón?
¿Resulta alarmante o más bien una señal de que el sistema ``está vivo''?
...
Si (todavía) no dispones de una placa FPGA, también puedes ejecutar el ejemplo del LED parpadeante.
Para ello usarás la simulación de Chisel. Para evitar un tiempo de simulación demasiado largo, cambia la
frecuencia de reloj en el código Chisel de 50000000 a 50000. Ejecuta la siguiente
...
Esto ejecutará el tester, que funciona durante un millón de ciclos de reloj.
La frecuencia de parpadeo depende de la velocidad de simulación, que a su vez depende de la
velocidad de tu ordenador. Por tanto, quizá necesites experimentar un poco
con la frecuencia de reloj supuesta para poder ver el LED parpadeando en la simulación.
...
En esta sección presentamos los componentes básicos del diseño digital:
los circuitos combinacionales y los biestables (\emph{flip-flops}).
Estos elementos esenciales pueden combinarse para construir circuitos más grandes e interesantes.
...
Los sistemas digitales, en general, utilizan señales binarias, lo que significa que un único bit o señal
solo puede tomar uno de dos valores posibles. Esos valores suelen denominarse 0 y 1. Sin embargo,
también empleamos los siguientes términos: bajo/alto (\emph{low/high}), falso/verdadero (\emph{false/true})
y desactivado/activado (\emph{deasserted/asserted}).
Estos términos se refieren a los mismos dos valores posibles de una señal binaria.

\section{Tipos y constantes en Chisel}
...
Chisel proporciona tres tipos de datos para describir conexiones, lógica combinacional y registros:
\code{Bits}, \code{UInt} y \code{SInt}. \code{UInt} y \code{SInt} extienden \code{Bits},
y los tres tipos representan un vector de bits. \code{UInt} otorga a ese vector de
bits el significado de un entero sin signo, y \code{SInt} el de un entero
con signo.\footnote{Al tipo \codefoot{Bits}, en la versión actual de Chisel, le faltan operaciones y
por lo tanto no resulta muy útil en el código final del usuario.}
Chisel utiliza el \myref{https://en.wikipedia.org/wiki/Two\%27s\_complement}{complemento a dos}
como representación de los enteros con signo.
Aquí está la definición de distintos tipos: un \code{Bits} de 8 bits, un entero sin signo de 8 bits y un entero
con signo de 10 bits:
...
\noindent La anchura de un vector de bits viene definida por un tipo \emph{Width} de Chisel (\code{Width}).
La siguiente expresión convierte el entero de Scala \code{n} a un \code{width} de Chisel,
que usamos para la definición del vector \code{Bits}:
...
\noindent Las constantes pueden definirse usando un entero de Scala y convirtiéndolo a un tipo de Chisel:
...
\noindent Las constantes también pueden definirse con una anchura, usando el tipo \code{Width} de Chisel:
...
\noindent Si la notación \code{3.U} y \code{4.W} te resulta un poco extraña, considérala una variante de una constante
entera con un tipo. Por ejemplo, \code{3L}, que representa una constante entera larga en C, Java y Scala.
...
\textbf{Posible fallo:} un error frecuente al definir constantes con una anchura concreta es olvidar el
especificador de anchura \code{.W}. Por ejemplo, \code{1.U(32)} \emph{no} define una constante de 32 bits de anchura
que represente el valor 1.
En su lugar, la expresión \code{(32)} se interpreta como una extracción del bit de la posición 32, lo que da como
resultado una constante de un solo bit con el valor 0. Lo cuál, probablemente no era la intención original del programador.
...
Chisel se beneficia de la inferencia de tipos de Scala y, en muchos lugares, la información de tipo puede omitirse.
Lo mismo vale para el ancho de \code{Bits}. En muchos casos, Chisel infiere automáticamente la anchura correcta.
Por eso, una descripción de hardware en Chisel es más concisa y más legible que en VHDL o
Verilog.
...
Para las constantes definidas en bases distintas de la decimal, la constante se define en una cadena con
una \code{h} delante para hexadecimal (base 16), una \code{o} para octal (base 8) y una \code{b}
para binario (base 2). El siguiente ejemplo muestra la definición de la constante 255 en distintas
bases. En este ejemplo omitimos la anchura en bits y Chisel infiere la anchura mínima en la que caben
las constantes, en este caso 8 bits.
...
\noindent El código anterior muestra cómo usar el guion bajo para agrupar los dígitos de la
cadena que representa una constante. El guion bajo se ignora.
...
Los caracteres que representan texto (en codificación \myref{https://en.wikipedia.org/wiki/ASCII}{ASCII})
también pueden usarse como constantes en Chisel:
...
Para representar valores lógicos, Chisel define el tipo \code{Bool}.
\code{Bool} puede representar un valor \emph{true} o \emph{false}.
El siguiente código muestra la definición del tipo \code{Bool} y la definición de
constantes \code{Bool}, convirtiendo las constantes booleanas de Scala \code{true}
y \code{false} en constantes \code{Bool} de Chisel.
...
Chisel utiliza los operadores del \myref{https://en.wikipedia.org/wiki/Boolean_algebra}{álgebra de Boole},
tal y como se definen en C, Java, Scala y en otros lenguajes de programación,
para describir circuitos combinacionales: \code{\&} es el operador AND y \code{|} es
el operador OR.
La siguiente línea de código define un circuito que combina las señales \code{a} y \code{b} con una puerta
\emph{and}, combina el resultado de la \emph{and} con la señal \code{c} mediante una puerta \emph{or} y 
al resultado final lo denomina \code{logic}.
...
\begin{figure}[h]
  \centering
  \includegraphics[scale=\scale]{figures/logic}
  \caption{Lógica para la expresión \code{(a \& b) | c}.
  Los cables pueden ser de un solo bit o de varios bits. La expresión de Chisel y el esquema son los mismos.}
  \label{fig:logic}
\end{figure}
...
\noindent La figura~\ref{fig:logic} muestra el esquema de esta expresión combinacional.
Cabe señalar que este circuito puede corresponder a un vector de bits y no solo a cables individuales
combinados con circuitos AND y OR.
...
En este ejemplo no definimos ni el tipo ni la anchura de la señal \code{logic}.
Ambos se infieren a partir del tipo y la anchura de la expresión.
Las operaciones lógicas estándar de Chisel son:
...
\noindent Las operaciones aritméticas usan los operadores estándar:
...
\noindent La anchura resultante de la operación es la mayor de las anchuras de los operandos en
la suma y la resta, la suma de ambas anchuras en la multiplicación y, normalmente,
la anchura del numerador en las operaciones de división y módulo.\footnote{
  Los detalles exactos están disponibles en la
  \ifbook
    especificación de FIRRTL\footnotemark
  \else
    \myref{https://github.com/chipsalliance/firrtl-spec/releases/latest/download/spec.pdf}{especificación de FIRRTL}.
  \fi
}
\ifbook
  \footnotetext{\url{https://github.com/chipsalliance/firrtl-spec/releases/latest/download/spec.pdf}}
...
Una señal también puede definirse primero como un \code{Wire} de algún tipo. Después podemos asignarle
un valor al cable con el operador de actualización \code{:=}.
...
\noindent Se puede extraer un subcampo desde la posición final hasta la inicial:
...
\noindent Los campos de bits se concatenan con el operador \code{\#\#}.\footnote{Ten en cuenta que existe una función \codefoot{Cat} que realiza la misma
operación con \codefoot{Cat(highByte, lowByte)}.}
...
La tabla~\ref{tab:operators} muestra la lista completa de operadores
(véanse también los \myref{https://www.chisel-lang.org/chisel3/docs/explanations/operators.html}{builtin operators}).
La precedencia de operadores de Chisel viene determinada por el orden de evaluación del circuito,
que sigue el \myref{https://docs.scala-lang.org/tour/operators.html}{Scala operator precedence}.
En caso de duda, siempre es buena práctica usar paréntesis.\footnote{La precedencia de operadores en
Chisel es un efecto secundario de la elaboración del hardware, cuando el árbol de nodos de hardware
se crea al ejecutar los operadores de Scala. La precedencia de operadores de Scala es parecida a la de
Java/C, pero no idéntica. Verilog tiene la misma precedencia de operadores que C, pero VHDL
tiene otra distinta. Verilog establece un orden de precedencia para las operaciones lógicas, mientras que en VHDL
esos operadores tienen la misma precedencia y se evalúan de izquierda a derecha.}

% Table~\ref{tab:functions} shows various functions defined on and for Chisel data types.
La tabla~\ref{tab:functions} muestra varias funciones definidas sobre y para los tipos de datos de Chisel.
...
\begin{table}[h]
 ...
 \renewcommand{\arraystretch}{1.3}
  \begin{tabular}{lll}
    ...
    \midrule
    \code{* / \%} & multiplicación, división, módulo & UInt, SInt \\
    \code{+ -} & suma, resta & UInt, SInt \\
    \code{=== =/=} & igual, distinto & UInt, SInt, devuelve Bool \\
    \code{> >= < <=} & comparación & UInt, SInt, devuelve Bool \\
    \multirow{2}{*}{\code{<< >>}} & desplazamiento a izquierda y a derecha & \multirow{2}{*}{UInt, SInt} \\
     & (con extensión de signo en SInt) & \\
    \code{\~} & NOT & UInt, SInt, Bool \\
    \code{\& | \^} & AND, OR, XOR & UInt, SInt, Bool \\
    \code{!} & NOT lógico & Bool \\
    \code{\&\& ||} & AND y OR lógicos & Bool \\
    \bottomrule
  \end{tabular}
  \caption{Operadores hardware definidos por Chisel.}
 \label{tab:operators}
\end{table}
...
\begin{table}[h]
 ...
 \renewcommand{\arraystretch}{1.3}
  \begin{tabular}{lll}
    ...
    \midrule
    \code{v.andR v.orR v.xorR} & reducción AND, OR, XOR & UInt, SInt, devuelve Bool \\
    \code{v(n)} & extracción de un único bit & UInt, SInt \\
    \code{v(end, start)} & extracción de un campo de bits & UInt, SInt \\
    \code{Fill(n, v)} & replicación de bits, n veces & UInt, SInt \\
    \code{a \#\# b} & concatenación de campos de bits & UInt, SInt \\
    \code{Cat(a, b, ...)} & concatenación de campos de bits & UInt, SInt \\
    \bottomrule
  \end{tabular}
  \caption{Funciones hardware definidas por Chisel, invocadas sobre \code{v}.}
   \label{tab:functions}
\end{table}
...
Un \myref{https://en.wikipedia.org/wiki/Multiplexer}{multiplexor} es un circuito que selecciona entre varias alternativas.
En su forma más básica, selecciona entre dos alternativas. La figura~\ref{fig:mux} muestra
uno de esos multiplexores 2:1, o \emph{mux} para abreviar. Según el valor de la
señal de selección (\code{sel}), la señal \code{y} representará la señal \code{a} o
la señal \code{b}.

\begin{figure}[h]
  \centering
  \includegraphics[scale=\scale]{figures/mux}
  \caption{Un multiplexor 2:1 básico.}
  \label{fig:mux}
\end{figure}

% A multiplexer can be built from logic.
% However, as multiplexing  is such a standard operation, Chisel provides a multiplexer,
Un multiplexor puede construirse a partir de lógica.
No obstante, como el multiplexado es una operación tan habitual, Chisel proporciona una clase para 
instanciar directamente un multiplexor,
...
\noindent donde se selecciona \code{a} cuando \code{sel} es \code{true.B}, y en caso contrario se
selecciona \code{b}. El tipo de \code{sel} es un \code{Bool} de Chisel; las entradas \code{a} y \code{b}
pueden ser cualquier tipo base de Chisel o agregado (\emph{bundles} o vectores), siempre que sean del
mismo tipo.
...
Mediante operaciones lógicas y aritméticas y un multiplexor, se puede describir cualquier
circuito combinacional. Sin embargo, Chisel ofrece componentes adicionales y abstracciones de control
que permiten una descripción más elegante de un circuito combinacional, tal y como se explica en
el capítulo~\ref{chap:comb}.
...
El segundo componente básico necesario para describir un circuito digital es un elemento de estado,
también llamado registro, que se describe a continuación.
...
Chisel también proporciona una clase para instanciar registros. Un registro es un conjunto de
\myref{https://en.wikipedia.org/wiki/Flip-flop\_(electronics)\#D\_flip-flop}{biestables de tipo D}.
El registro se conecta por defecto a una señal de reloj global y se actualiza su estado en el flanco de subida.
Cuando se proporciona un valor de inicialización en la declaración del registro,
este usa un reset síncrono conectado a una señal global de reset.
Un registro puede ser cualquier tipo de Chisel que pueda representarse como un conjunto de bits.
El siguiente código define un registro de 8 bits, inicializado a 0 en el reset:
...
\noindent Una entrada se conecta al registro con el operador de actualización \code{:=}, y
la salida del registro puede usarse en una expresión utilizando directamente su nombre:

\shortlist{code/reg_con.txt}

% \noindent A register can also be connected to its input at the definition:

% \noindent Un registro también puede conectarse a su entrada en la propia definición:
\noindent A un registro tambíen se le puede conectar directamente su entrada en la propia definición:
...
La figura~\ref{fig:register-reset-0} muestra el circuito del registro definido anteriormente, con
un reloj, un reset síncrono a \code{0.U}, la entrada \code{d} y la salida \code{q}.
Las señales globales \code{clock} y \code{reset} se conectan implícitamente a
cada registro que se define.
...
\begin{figure}[H]
  \centering
  \includegraphics[scale=\scale]{figures/register-reset-0}
  \caption{Un registro basado en biestables D con reset síncrono a 0.}
  \label{fig:register-reset-0}
\end{figure}
...
\noindent A un registro también se le puede conectar, en la propia definición, la entrada y asignarle una
constante como valor inicial:
...
\noindent Para distinguir entre las señales que representan lógica combinacional y los registros,
una práctica habitual es añadir el sufijo \code{Reg} a los nombres de los registros.
Otra práctica habitual, heredada de Java y Scala, es usar
\myref{https://en.wikipedia.org/wiki/Camel_case}{CamelCase} para
los identificadores formados por varias palabras. El convenio es empezar
las funciones y las variables con minúscula, y las clases (los tipos),
por ejemplo el nombre de un \code{Module}, con mayúscula.
...
En Chisel tienes bastante libertad a la hora de nombrar tus identificadores. Sin embargo, se recomienda usar 
nombres descriptivos y el sentido común. Además, hay varias palabras reservadas, que están recogidas
en el apéndice~\ref{sec:reserved}.
...
Contar es una operación fundamental en los sistemas digitales. Se pueden contar sucesos,
pero lo más habitual es que contar sirva para definir un intervalo de tiempo: se cuentan
los ciclos de reloj y se dispara una acción cuando el intervalo ha terminado.
...
y en diseño digital se empieza a contar en 0. Por lo tanto, si queremos contar
10 ciclos de reloj, contamos de 0 a 9. El siguiente código muestra uno de esos contadores, que cuenta
hasta 9 y vuelve a 0 al llegar a 9.

\shortlist{code/counter.txt}

\section{Estructuras con Bundle y Vec}
...
Chisel proporciona dos clases para agrupar señales relacionadas: (1) un \code{Bundle} y (2) un \code{Vec}.
Un \code{Bundle} agrupa señales de distintos tipos como campos con nombre.
Un \code{Vec} representa una colección indexable de señales (elementos) del mismo tipo.
Los \code{Bundle} y los \code{Vec} crean tipos de Chisel nuevos, definidos por el usuario, y pueden anidarse
de forma arbitraria.
...
Un \code{Bundle} de Chisel agrupa varias señales. Se puede hacer referencia al bundle entero
como un todo, o acceder a cada campo por su nombre.
Un \code{Bundle} es parecido a un \code{struct} de C y SystemVerilog, o a un \code{record} de VHDL.
Podemos definir un bundle (una colección de señales) definiendo una clase que
extienda \code{Bundle} y enumerando los campos como \code{val} dentro del bloque del constructor.
...
\noindent Para usar un bundle, lo creamos con \code{new} y lo envolvemos en un \code{Wire}.
A los campos se accede con la notación de punto:
...
La notación de punto es habitual en los lenguajes orientados a objetos, donde \code{x.y} significa
que \code{x} es una referencia a un objeto e \code{y} es un campo de ese objeto.
Como Chisel está orientado a objetos, usamos la notación de punto para acceder a los
\newpage 
...
Un \code{Vec} de Chisel (un vector) representa una colección de elementos de Chisel del mismo tipo.
A cada elemento se accede mediante un índice. Un \code{Vec} de Chisel es parecido
a las estructuras de datos de tipo array de otros lenguajes de programación.\footnote{El nombre
\codefoot{Array} ya se usa en Scala.}
...
Un \code{Vec} se usa para tres fines distintos: (1) el direccionamiento dinámico en hardware,
con esto nos referimos básicamente a un multiplexor; (2) un banco de registros, que incluye el multiplexado 
de la lectura y la generación de la señal de habilitación (\emph{enable}) para la escritura; (3) la 
parametrización del número de puertos de un \code{Module}. 

Para otras colecciones de \emph{cosas}, ya sean 
componentes hardware u otros datos del generador, es mejor usar la colección \code{Seq} de Scala.

\subsubsection{Vec combinacional}
...
Un \code{Vec} se crea llamando al constructor con dos parámetros: el
número de elementos y el tipo de los elementos. Un \code{Vec} combinacional
necesita envolverse en un \code{Wire}:
...
\noindent A los elementos individuales se accede con \code{(index)}.
Un vector envuelto en un \code{Wire} no es más que un multiplexor.
...
Aquí tenemos otro ejemplo de uso de un \code{Vec} como multiplexor.
Las tres entradas se conectan a los tres cables \code{x}, \code{y}
y \code{z}. El cable \code{select} elige qué entrada se usa y
la conecta a \code{muxOut}.

\shortlist{code/vec_mux.txt}

% \noindent Figure~\ref{fig:vec-mux} shows the resulting schematic of the above code snippet.

\noindent La figura~\ref{fig:vec-mux} muestra el esquema resultante del fragmento de código anterior.
...
\begin{figure}[H]
  \centering
  \includegraphics[scale=\scale]{figures/vec-mux}
  \caption{Un vector envuelto en un \code{Wire} no es más que un multiplexor.}
  \label{fig:vec-mux}
\end{figure}
...
De forma parecida a como se usa un \code{WireDefault}, podemos fijar los valores por defecto de un
\code{Vec} con \code{VecInit}.
El siguiente código representa un multiplexor 3:1 con tres constantes por defecto. Fíjate en que
especificamos el tamaño (3 bits) de los tipos de datos \code{UInt} con la primera constante.
Con la condición (\code{cond}) podemos sobrescribir esos valores por defecto.
Ese hardware de sobrescritura consiste, a su vez, en tres multiplexores 2:1. La última línea selecciona
una de las tres entradas del multiplexor \code{defVec}. Ten en cuenta que \code{VecInit} ya devuelve
hardware de Chisel, así que no necesitamos envolverlo en un \code{Wire}.\footnote{Esto es distinto de
un \codefoot{Vec} normal, que sí necesita envolverse en un \codefoot{Wire}. Podríamos envolver el
\codefoot{VecInit} en un \codefoot{WireDefault}, pero es un estilo de programación poco habitual.}
...
No solo es posible fijar constantes iniciales (como en \code{WireDefault}) para la entrada del \code{Vec};
también podemos conectar señales (cables) con \code{VecInit} a las entradas del \code{Vec}.
El siguiente ejemplo conecta los cables \code{d}, \code{e} y \code{f} a las tres
entradas del \code{Vec}.

\shortlist{code/vec_init_sig.txt}


\subsubsection{Vec de registros}
...
También podemos envolver un \code{Vec} en un registro para definir un array de registros.
El siguiente código muestra un vector de tres registros.
...
La figura~\ref{fig:vec-reg} muestra el esquema de ese circuito, que contiene tres registros.
El índice de lectura (\code{rdIdx}) gobierna el multiplexor conectado a la salida de los tres
registros. La señal de salida es \code{dout}.
El índice de escritura (\code{wrIdx}) selecciona en qué registro se escribirán los datos
procedentes de \code{din}. \code{wrIdx} ataca a un decodificador que activa una de las tres
señales de habilitación de los registros.

\begin{figure}[H]
  \centering
  \includegraphics[scale=0.525]{figures/vec-reg}
  \caption{Un vector de registros.}
  \label{fig:vec-reg}
\end{figure}
...
El siguiente ejemplo define un banco de registros para un procesador: 32 registros
de 32 bits cada uno, como los que se usan en un procesador
\myref{https://en.wikipedia.org/wiki/Reduced_instruction_set_computer}{RISC}
clásico de 32 bits, como la versión de 32 bits de \myref{https://en.wikipedia.org/wiki/RISC-V}{RISC-V}.
...
\noindent A un elemento de ese banco de registros se accede con un índice y se usa como un registro normal.
...
Un registro de un vector también puede inicializarse. Ese es el valor al que se pone el registro en el reset.
Para inicializar el banco de registros se utilizan constantes haciendo uso de un \code{VecInit}, envuelto
en un \code{RegInit}. Las entradas de los tres registros se conectan después a los cables \code{d},
\code{e} y \code{f}.
...
Si queremos poner todos los elementos de un banco de registros grande al mismo valor en el reset
(probablemente ese valor sea 0), podemos usar una secuencia \code{Seq} de Scala. Un \code{VecInit} puede 
construirse con una secuencia que contenga tipos de Chisel. \code{Seq} dispone de una función de creación,
\code{fill}, que inicializa una secuencia con valores idénticos.
El siguiente código construye un banco de registros con 32 registros, cada uno de 32 bits y con
el reset a 0:

\shortlist{code/reg_vec_reset.txt}

\subsubsection{Combinar Bundle y Vec}
...
Podemos mezclar libremente bundles y vectores. Al crear un vector con un tipo
bundle, necesitamos pasar un prototipo del tipo para poder definir los campos del vector. 
Usando el ejemplo \code{Channel}, que definimos más arriba, podemos crear un vector de canales con:

\shortlist{code/vec_bundle.txt}

% \noindent A bundle may as well contain a vector:

\noindent Del mismo modo, un bundle puede contener un vector:
...
Cuando queremos un registro de un tipo bundle que necesita un valor de reset,
primero creamos un \code{Wire} de ese bundle, damos a cada campo el valor
que corresponda y después pasamos ese bundle a un \code{RegInit}:
...
Combinando \code{Bundle} y \code{Vec} podemos definir nuestras propias estructuras
de datos, que son abstracciones muy potentes.
...
\textbf{Posible fallo:} en Chisel 3 no se permiten las asignaciones parciales, aunque sí estaban
soportadas en Chisel 2 y son posibles en Verilog y VHDL. El siguiente código generará
un error durante la elaboración del circuito:
...
\noindent El argumento es que, para este caso de uso, es preferible utilizar bundles.
Una posible solución consiste en crear un bundle (local), crear un \code{Wire}
a partir de ese bundle, asignar cada uno de los campos, convertir ese bundle a un
\code{UInt} con \code{asUInt} y asignar ese valor al \code{UInt} de destino.
Fíjate en que aquí definimos un \code{Bundle} como una estructura de datos local, ya que solo
lo necesitamos ahí.
...
\noindent El pequeño inconveniente de esta solución es que hay que saber en qué orden se
combinan los campos del bundle para formar un único vector de bits. Otra opción es usar un vector de
\code{Bool} para asignar los valores uno a uno y convertirlo después a un \code{UInt}.

\shortlist{code/partial_vec_bool.txt}

\section{Wire, Reg e IO}
\index{Wire}
\index{Reg}
\index{IO}
...
\code{UInt}, \code{SInt} y \code{Bits} son tipos de Chisel que por sí solos no
representan hardware. Solo se genera hardware al envolverlos en un \code{Wire}, un \code{Reg}
o un \code{IO}. Un \code{Wire} representa lógica combinacional, un \code{Reg}
representa un registro (un conjunto de biestables D) y un \code{IO} representa los puertos
para la conexión de un módulo (como las patillas, o pines, de un circuito integrado concreto).
Cualquier tipo de Chisel puede envolverse en un \code{Wire}, un \code{Reg} o un \code{IO}.
...
A un componente hardware se le da nombre asignándolo a una variable inmutable de
Scala:\footnote{Scala también admite variables mutables con \codefoot{var}, pero no sirven de nada
al describir hardware en Chisel.}
...
\noindent Más adelante puedes asignar (o reasignar) un valor o una expresión a un \code{Wire}, a un \code{Reg}
o a un \code{IO} con el operador \code{:=} de Chisel
...
\noindent Fíjate en la pequeña diferencia que hay entre el operador de asignación ``\code{=}'' de Scala
y el operador ``\code{:=}'' de Chisel. El operador ``\code{=}'' de Scala se usa al \emph{crear} un objeto
hardware (y darle un nombre), mientras que el operador ``\code{:=}'' de Chisel se usa al asignar o reasignar
un valor a un objeto hardware \emph{ya existente}.
...
Los valores combinacionales pueden asignarse de forma condicional, pero hay que asignarlos en todas las
ramas de la condición. De lo contrario se estaría describiendo un \emph{latch}, algo que el compilador de
Chisel rechazará.
La mejor práctica es definir un valor por defecto al crear el \code{Wire}.
Por lo tanto, es mejor reescribir el código anterior de la siguiente manera.
...
\noindent Aunque Chisel infiere la anchura en bits necesaria para las señales y los registros, es una
buena práctica especificar la anchura en bits que se pretende al crear el objeto hardware.
En la mayoría de los casos también es una buena práctica poner los registros a valores iniciales conocidos en el
reset:\footnote{Dejar el valor del registro indefinido en el reset puede aliviar algo de carga en el cable de
reset. Sin embargo, las pruebas y la verificación se simplifican con valores de reset conocidos.}

\shortlist{code/wire_reg_init.txt}

\section{Chisel genera hardware}
...
Después de ver algo de código Chisel, puede parecer similar a lenguajes de programación
clásicos como Java o C. Sin embargo, Chisel (o cualquier otro lenguaje de descripción de
hardware) lo que hace es definir componentes hardware. Mientras que en un programa software se
ejecuta una línea de código detrás de otra, en hardware todas las líneas de código
\emph{se ejecutan en paralelo}.
...
Es fundamental tener presente que el código Chisel genera hardware.
Intenta imaginar, o dibujar en una hoja de papel, los bloques funcionales concretos que
genera tu descripción del circuito en Chisel.
Cada componente que se crea añade hardware; cada sentencia de asignación
genera puertas y/o biestables.
...
En términos más técnicos, cuando Chisel ejecuta tu código lo hace como un programa de Scala y,
al ejecutar las sentencias de Chisel, \emph{recopila} los componentes hardware
y conecta esos nodos. Esa red de nodos hardware es el hardware,
que Chisel puede volcar como código Verilog para la síntesis en un ASIC o en una FPGA, o que
puede probarse con un tester de Chisel.
Esa red de nodos hardware es lo que se ejecuta totalmente en paralelo.
...
Si eres ingeniero de software, imagina el inmenso paralelismo que puedes
crear en hardware sin necesidad de dividir tu aplicación en hilos de ejecución (\emph{threads})
ni de gestionar los bloqueos para la comunicación entre ellos.
...
En la introducción implementaste un LED parpadeante en una placa FPGA
(a partir de \myref{https://github.com/schoeberl/chisel-examples}{chisel-examples}), que es un ejemplo
razonable de \emph{Hola Mundo} en hardware. Solo usaba estado interno, una única salida al LED y ninguna
entrada.
Copia ese proyecto en una carpeta nueva y amplíalo añadiendo algunas entradas al \code{Bundle} \code{io}
con \code{val sw = Input(UInt(2.W))}.
...
\noindent Para esos interruptores también necesitas asignar los pines de la placa FPGA.
Puedes encontrar ejemplos de asignaciones de pines en el proyecto de la ALU en Quartus (por ejemplo, para la
\myref{https://github.com/schoeberl/chisel-examples/blob/master/quartus/altde2-115/alu.qsf}{placa FPGA
DE2-115}).
...
Cuando hayas definido esas entradas y la asignación de pines, empieza con una prueba sencilla:
elimina del diseño toda la lógica de parpadeo y conecta un interruptor a la salida del LED;
compila y configura el dispositivo FPGA. ¿Puedes encender y apagar el LED con el interruptor?
Si es así, ya tienes entradas disponibles. Si no, tendrás que depurar la configuración de la FPGA.
La asignación de pines también puede hacerse con la interfaz gráfica de usuario de la herramienta.
...
Usa ahora dos interruptores e implementa una de las funciones combinacionales básicas.
Por ejemplo, haz la AND de los dos interruptores y muestra el resultado en el LED. Después puedes probar con otras funciones.
El siguiente paso consiste en usar tres interruptores de entrada para implementar un multiplexor: uno hace
de señal de selección y los otros dos son las dos entradas del multiplexor 2:1.
...
Ya has podido implementar funciones combinacionales sencillas y probarlas
en hardware real en una FPGA. Como siguiente paso, veremos por primera vez cómo funciona el proceso
de construcción que genera la configuración de la FPGA. Además, también
exploraremos un sencillo entorno de pruebas de Chisel que te permite probar circuitos
sin configurar una FPGA ni accionar interruptores.

\chapter{Proceso de compilación y pruebas}
...
Para comenzar con código Chisel más interesante, primero necesitamos aprender a compilar
programas en Chisel, a generar código Verilog para ejecutarlo en una FPGA
y a escribir pruebas que sirvan para depurar y para verificar que nuestros circuitos son correctos.
...
Chisel está escrito en Scala, así que con un proyecto de Chisel es posible usar cualquier proceso de
compilación que soporte Scala. Una herramienta de compilación popular para Scala es
\myref{https://www.scala-sbt.org/}{sbt}, siglas de \emph{Scala interactive build tool}.
Además de dirigir el proceso de compilación y de pruebas, \code{sbt} también descarga la versión
correcta de Scala y de las bibliotecas de Chisel.

\section{Compilar tu proyecto con sbt}
...
La biblioteca de Scala que representa a Chisel y el tester de Chisel se descargan automáticamente
de un repositorio Maven durante el proceso de compilación.
Las bibliotecas se referencian desde \code{build.sbt}. Es posible configurar \code{build.sbt}
con \code{latest.release} para usar siempre la versión más reciente de Chisel.
Sin embargo, eso significa que en cada compilación hay que consultar la versión en el repositorio
Maven, y esa consulta necesita conexión a Internet para que la compilación tenga éxito.
Es mejor fijar en tu \code{build.sbt} una versión concreta de Chisel y del resto de bibliotecas de Scala.
A veces viene bien poder escribir código hardware y probarlo sin conexión a Internet.
Por ejemplo, resulta estupendo hacer diseño hardware en un avión.
...
\code{sbt} hereda los convenios de organización del código fuente de la herramienta de automatización para
la compilación \myref{https://maven.apache.org/}{Maven}. Además, Maven organiza repositorios de bibliotecas
de código abierto en Java.\footnote{Ese es también el sitio del que descargaste la biblioteca de Chisel en
tu primera compilación: \url{https://mvnrepository.com/artifact/edu.berkeley.cs/chisel3}.}
...
\caption{Estructura de un proyecto en Chisel (usando \code{sbt}). También denominada árbol de ficheros fuentes.}
\label{fig:folders}
\end{figure}
...
La figura~\ref{fig:folders} muestra la organización de los ficheros de código fuente en un proyecto típico de Chisel.
La raíz del proyecto es la carpeta principal, que contiene el \code{build.sbt}.
También puede incluir un \code{Makefile} para el proceso de compilación, un README y un fichero LICENSE.
La carpeta \code{src} contiene todo el código fuente y, a partir de ahí, se divide entre \code{main},
que contiene los ficheros fuentes del hardware, y \code{test}, que contiene los testers.
En ambos casos la carpeta siguiente es \code{scala}, ya que Chisel se basa en Scala.
Si quieres incluir código Java, que puede resultar útil para los generadores de hardware,
añadirías una carpeta \code{java}.
Chisel hereda de Scala, que a su vez lo hereda de Java, la organización del código fuente en
\myref{https://en.wikipedia.org/wiki/Java_package}{paquetes}.
Los paquetes organizan tu código Chisel en espacios de nombres (\emph{namespaces}). Los paquetes pueden contener
a su vez subpaquetes.
La carpeta \code{target} contiene los ficheros de clases y otros ficheros generados.
Recomiendo usar también una carpeta para los ficheros Verilog generados, que suele
llamarse \code{generated}.
...
Para aprovechar los espacios de nombres en Chisel, es necesario declarar que una clase o módulo
está definido en un paquete, en este ejemplo en \code{mypack}:
...
\noindent Fíjate en que en este ejemplo vemos la importación del paquete \code{chisel3}
para poder usar las clases de Chisel.
...
Para usar el módulo \code{Abc} en otro contexto (en otro espacio de nombres de paquete),
hay que importar los componentes del paquete \code{mypack}. El guion bajo
(\code{\_}) actúa como comodín, lo que significa que se importan todas las clases de \code{mypack}.
...
\noindent También es posible no importar todos los tipos de \code{mypack} y
usar en su lugar el nombre completo \code{mypack.Abc} para referirse al módulo
\code{Abc} del paquete \code{mypack}.

\shortlist{code/usemypack2.txt}

% \noindent It is also possible to import just a single class and create an instance of it:

\noindent También es posible importar una única clase y crear una instancia de ella:

\shortlist{code/usemypack3.txt}


\subsection{Ejecutar sbt}

% A Chisel project can be compiled and executed with a simple \code{sbt} command:

Un proyecto de Chisel puede compilarse y ejecutarse con un simple comando de \code{sbt}:
...
Este comando compilará todo tu código Chisel de los ficheros fuentes y buscará
las clases que contengan un \code{object} que tenga un método \code{main} o que
extienda \code{App}. Si hay más de un objeto de ese tipo, se listan todos y
puede elegirse uno.
También puedes indicar directamente el objeto que quieres ejecutar como parámetro de \code{sbt}:
...
Por defecto, \code{sbt} solo busca en la parte \code{main} del árbol de ficheros fuentes, y no en la
parte \code{test}.\footnote{Es un convenio de Java/Scala que la carpeta de pruebas contenga
pruebas unitarias y no objetos con un \codefoot{main}.}
Para ejecutar las pruebas basadas en ChiselTest basta con lanzarlas con
...
Si tienes una prueba que no sigue el convenio de ChiselTest y contiene una función \code{main},
pero está colocada en la parte \code{test} del árbol de ficheros fuentes, puedes ejecutarla
con el siguiente comando de \code{sbt}:

\begin{verbatim}
$ sbt "test:runMain mypacket.MyMainTest"
\end{verbatim}

\subsection{Generar Verilog}
...
Para poder sintetizar código Chisel teniendo como objetivo una FPGA o un ASIC necesitamos traducir Chisel a
un lenguaje de descripción de hardware que la herramienta de síntesis entienda.
Con Chisel podemos generar una descripción sintetizable del circuito en Verilog.
...
Para generar la descripción en Verilog necesitamos una aplicación. Un objeto de Scala que haga
\code{extends App} es una aplicación que genera implícitamente la función main, que es el punto por el que se 
inicia la aplicación.
En el ejemplo \code{Hello}, la única acción de la aplicación es crear un nuevo módulo de Chisel, y pasárselo a 
la función \code{emitVerilog()} de Chisel. El siguiente código generará el fichero Verilog \code{Hello.v}.
...
Si se usa la versión por defecto de \code{emitVerilog()}, los ficheros generados se dejan
en la carpeta raíz de nuestro proyecto (donde ejecutamos el comando \code{sbt}).
Para dejarlos en una subcarpeta hay que indicárselo a \code{emitVerilog()} mediante opciones.
Recomiendo indicar una carpeta \code{generated}, como se muestra en la figura~\ref{fig:folders}.
Las opciones de compilación se pueden pasar como segundo argumento, el cual es un array de \code{String}.
El siguiente código generará el fichero Verilog \code{Hello.v} en la subcarpeta \code{generated}.
...
También puedes pedir el código Verilog como un \code{String} de Scala, sin escribir ningún fichero.
Para hacer pruebas basta con imprimir esa cadena por pantalla.
...
\noindent Esta forma de salida es habitual al mostrar pequeños ejemplos de Chisel en
\myref{https://scastie.scala-lang.org/}{Scastie}, un compilador y entorno de ejecución de Scala en la web.
Como ejemplo, véase
\myref{https://scastie.scala-lang.org/schoeberl/SN7rDb9iS027ORiWqXMGsQ/6}{Hello World en Scastie}.


\subsection{Flujo de herramientas}
...
\begin{figure}
  \centering
  \includegraphics[scale=\scale]{figures/flow}
  \caption{Flujo de herramientas del ecosistema de Chisel.}
  \label{fig:flow}
\end{figure}
...
La figura~\ref{fig:flow} muestra el flujo de herramientas de Chisel. El circuito digital se describe en una
clase de Chisel, que aparece como \code{Hello.scala}. El compilador de Scala compila esa clase, junto con
las bibliotecas de Chisel y de Scala, y genera el fichero de clase de Java \code{Hello.class}, que puede
ejecutar cualquier
\myref{https://en.wikipedia.org/wiki/Java_virtual_machine}{máquina virtual de Java (JVM)} estándar.
Al ejecutar esa clase con un controlador de Chisel se genera la representación intermedia flexible para
RTL (FIRRTL), una representación intermedia de los circuitos digitales. En nuestro ejemplo, el fichero es
\code{Hello.fir}.
El compilador de FIRRTL realiza transformaciones sobre el circuito.
...
Treadle es un intérprete de FIRRTL capaz de simular el circuito. Junto con el tester de Chisel, puede
usarse para depurar y probar circuitos en Chisel. Mediante aserciones (\emph{assertions}) podemos obtener 
los resultados de las pruebas.
Treadle también puede generar ficheros de formas de onda (\code{Hello.vcd}) que pueden verse con
...
Una de las transformaciones de FIRRTL, el emisor de Verilog, genera el código Verilog para la síntesis
(\code{Hello.v}).
Una herramienta de síntesis de circuitos (por ejemplo, Intel Quartus, AMD/Xilinx Vivado o una herramienta
para ASIC) sintetiza el circuito.
En un flujo de diseño para FPGA, la herramienta genera el \emph{bitstream} que se usa para configurar la
FPGA, por ejemplo \code{Hello.bit}.
...
Ten en cuenta que la figura~\ref{fig:flow} muestra el flujo de herramientas de Chisel hasta la versión 3.6.
Las versiones posteriores de Chisel usan un backend distinto (véase la sección siguiente).
...
Ahora que conocemos la estructura básica de un proyecto de Chisel y cómo compilarlo y ejecutarlo
con \code{sbt}, podemos continuar viendo un sencillo entorno de pruebas.

\subsection{Versiones de Chisel}
...
A partir de la versión 5.0.0, Chisel cambió su esquema de versiones a un versionado semántico con la
forma MAJOR.MINOR.PATCH. Antes de Chisel 5.0.0 el esquema era 3.MAJOR.MINOR.
Los cambios en la versión menor de Chisel aportan nuevas funcionalidades, pero siguen siendo retrocompatibles.
Un cambio en la versión mayor puede introducir cambios que rompan la compatibilidad.
Para recalcar ese cambio en las versiones, Chisel se saltó la versión 4.
...
A partir de Chisel 5, el backend de Chisel ha pasado del compilador de FIRRTL basado en Scala a usar
\code{firtool} para generar Verilog. \code{firtool} forma parte de un proyecto mayor,
el proyecto \myref{https://circt.llvm.org/}{LLVM CIRCT}.
La última versión de ``Chisel 3'' es la 3.6 y contiene los dos backends: el compilador basado en
Scala y \code{firtool}.
El \code{build.sbt} para la 3.6 es el siguiente:
...
Con la versión 5 de Chisel, el binario de \code{firtool} hay que instalarlo a mano.
Chisel 6 incluye \code{firtool} dentro de la propia biblioteca de Chisel y es, por tanto, la última
versión recomendada de Chisel.
...
El cambio a CIRCT tiene consecuencias para la futura infraestructura de pruebas.
Las pruebas se harán en el futuro con una API de Scala construida sobre
\myref{https://www.veripool.org/verilator/}{Verilator}, lo que hace obligatorio instalar
Verilator. No obstante, dentro del proyecto CIRCT se está trabajando en poder simular con el propio
entorno de CIRCT. Esa herramienta se llama Arcilator.
En cualquier caso, ChiselTest se ha portado a Chisel 5 y a Chisel 6.
Esperamos que Chisel 7 incluya una capa de compatibilidad que permita una transición suave a un nuevo
entorno de pruebas.
...
A partir de Chisel 5, las bibliotecas se publican bajo \code{org.chipsalliance}.
Por tanto, el \code{build.sbt} debe cambiarse de la siguiente manera:
...
\subsubsection{Versiones de Chisel, Scala y Java}
...
Chisel es una biblioteca de Scala y, por tanto, depende de una versión de Scala.
Scala, a su vez, está construido sobre Java (usa la biblioteca de Java y se ejecuta sobre la JVM)
y depende por ello de una versión concreta de Java. Los cambios en la versión de Java exigen
actualizar la versión de parche (\emph{patch}) de Scala. Una versión de Java segura como punto de partida es Java 8,
con la que funcionan todas las versiones de Scala y de
Chisel.\footnote{Véase \url{https://docs.scala-lang.org/overviews/jdk-compatibility/overview.html}}
Chisel se ha mantenido de forma que funcione con Scala 2.12 y 2.13 hasta la versión 5.
Chisel 6 se basa únicamente en Scala 2.13.\footnote{Conseguir que Chisel funcione sobre Scala 3 es un
trabajo en curso.}
Una elección segura es usar Scala 2.13. Ahora bien, como Chisel usa un plugin del compilador de Scala,
la versión de parche más alta de Scala que se admite viene limitada por el plugin del compilador
publicado.\footnote{Véase \url{https://mvnrepository.com/artifact/edu.berkeley.cs/chisel3-plugin}
y \url{https://mvnrepository.com/artifact/org.chipsalliance/chisel-plugin}}
La tabla~\ref{tab:versions} muestra las versiones de Chisel con las versiones más altas de
Scala y de Java que admiten.
...
\begin{table}[H]
 \centering
  \begin{tabular}{lll}
    ...
    \bottomrule
  \end{tabular}
  \caption{Versiones de Chisel y versiones más altas de Scala y de Java admitidas.}
 \label{tab:versions}
\end{table}
...
\subsection{Usar una plantilla de GitHub}
...
El proyecto \myref{https://github.com/schoeberl/chisel-empty}{chisel-empty} contiene lo mínimo que un proyecto de
Chisel necesita. Y en particular contiene un circuito sumador de ejemplo, un tester y un \code{Makefile} para generar
el código Verilog, probar el circuito y limpiar el repositorio.
Ese proyecto es una plantilla de GitHub, por lo que puedes crear un nuevo repositorio de 
GitHub utilizándola.
...
Ve al repositorio de GitHub y pulsa el botón ``Use this template'' para crear
tu proyecto de GitHub. Después clona tu nuevo proyecto en local y explora el \code{Makefile}.
...
\noindent Elimina todos los ficheros generados con:
...
\noindent También puedes realizar estas tareas ejecutando directamente los comandos de \code{sbt} y de
\code{git}.

\section{Pruebas con Chisel}
\index{Testing}
...
Las pruebas de los diseños de hardware suelen llamarse bancos de pruebas
(\myref{https://en.wikipedia.org/wiki/Test_bench}{\emph{test benches}}).
El banco de pruebas instancia el diseño bajo prueba (DUT, \emph{design under test}), excita los puertos de
entrada, observa los puertos de salida y los compara con los valores esperados.
Chisel proporciona \myref{https://www.chisel-lang.org/chiseltest/}{ChiselTest}
en el paquete \code{chiseltest}.
...
Uno de los puntos fuertes de Chisel es que puede usar toda la potencia de Scala para escribir
bancos de pruebas. Se puede, por ejemplo, programar la funcionalidad esperada del hardware
en un simulador software y comparar la simulación del hardware con la
simulación software. Este método resulta muy eficaz al probar la implementación
de un procesador~\cite{lipsi:arcs2018}.
...
\myref{http://www.scalatest.org/}{ScalaTest} es una herramienta de pruebas para Scala (y Java). ChiselTest es
una extensión de ScalaTest, así que primero veremos un ejemplo sencillo de ScalaTest. Para usarlo, incluye
la biblioteca en tu \code{build.sbt} con la siguiente línea:
...
\noindent Las pruebas suelen estar en \code{src/test/scala}, y el conjunto entero de pruebas puede
ejecutarse con:
...
\noindent Una prueba mínima (un hola mundo de las pruebas) que comprueba una suma y una
multiplicación de enteros de Scala tiene este aspecto:
...
Con ScalaTest, unas pruebas unitarias sencillas se convierten en una especificación que además se
ejecuta. El ejemplo anterior enuncia dos pruebas como si fueran frases (``Integers should add''), y al
ejecutarlas la salida las repite una por una para indicar que las dos se han superado:
...
\code{sbt test} ejecuta todas las pruebas disponibles, lo que resulta útil para las pruebas de
regresión.\footnote{Prueba a ejecutar \codefoot{sbt test} en el repositorio de este libro y verás
pasar más de 90 pruebas.}
Ahora bien, si solo quieres ejecutar una prueba (o un conjunto de pruebas), puedes hacerlo con:
...
\noindent Si escribes mal el nombre de la clase, por ejemplo \code{ExampeTest},
obtendrás un mensaje de error bastante discreto: \code{No tests were executed}.
...
\myref{https://www.chisel-lang.org/chiseltest/}{ChiselTest} es la herramienta estándar para probar
módulos de Chisel. Se apoya en \myref{http://www.scalatest.org/}{ScalaTest}, la herramienta para Scala
y Java, que es la que nos permite lanzar las pruebas de Chisel.
Para usarla, incluye la biblioteca \code{chiseltest} en tu \code{build.sbt} con la siguiente línea:
...
\noindent Al incluir ChiselTest de esta manera se incluye automáticamente la versión necesaria de
ScalaTest, así que no hace falta añadir una línea para la biblioteca de ScalaTest.
Para usar ChiselTest hay que importar los siguientes paquetes:
...
\noindent Probar un circuito supone (al menos) dos componentes: el dispositivo bajo prueba (llamado
a menudo DUT) y la lógica de prueba, también llamada banco de pruebas. Las pruebas se lanzan con
\code{sbt test}. No hace falta ningún objeto con una función \code{main}.
...
El siguiente código muestra nuestro sencillo diseño bajo prueba. Contiene dos puertos de entrada
(de 2 bits de anchura) y dos puertos de salida, uno de 2 bits y otro \code{Bool}. El circuito hace la AND
bit a bit de sus entradas \code{a} y \code{b}, saca el resultado por \code{out} y comprueba si las dos
señales son iguales:
...
El banco de pruebas de este DUT extiende \code{AnyFlatSpec} con \code{ChiselScalatestTester}, que es
quien aporta la funcionalidad de ChiselTest dentro de ScalaTest.
El método \code{test()} se invoca con el DUT como parámetro y el código de la prueba como función literal.
...
A los puertos de entrada y de salida del DUT se accede con \code{dut.io}.
Puedes fijar valores con un \code{poke} sobre un puerto, al que se le pasa como parámetro el valor
expresado con el tipo de Chisel de ese puerto de entrada.
Un puerto de salida se lee invocando \code{peekInt()} o \code{peekBoolean()} sobre él,
que devuelven el valor como un tipo de Scala.
El tester hace avanzar la simulación un ciclo de reloj con \code{dut.clock.step()}.
Para avanzar varios ciclos de reloj podemos pasarle un parámetro a
\code{step()}.
Podemos imprimir los valores de las salidas con \code{println()}.
...
\noindent verás los resultados impresos en el terminal
...
Vemos que 0 AND 1 da 0, y que 3 AND 2 da 2.
Además de inspeccionar a mano lo que se imprime, que es un buen punto de partida, podemos
expresar nuestras expectativas en el propio banco de pruebas invocando
\code{expect(valor)} sobre el puerto de salida, con el valor esperado como parámetro.
El siguiente ejemplo muestra cómo realizar pruebas con valores esperados:
...
Al ejecutar esta prueba no se imprime ningún valor generado por el hardware. Se imprime que todas las pruebas se han
superado, ya que todos los valores esperados son correctos.
...
Una prueba que falla, ya sea porque el error está en el DUT o en el banco de pruebas,
produce un mensaje de error que describe la diferencia entre el valor esperado y el valor
real. A continuación hemos cambiado el banco de pruebas para que espere un 4, que es incorrecto:
...
La función \code{peek()} devuelve un tipo de Chisel, que habría que convertir para usarlo
como tipo de Scala. Para facilitar el uso de los valores de prueba en el mundo de Scala, ChiselTest
ofrece \code{peekInt()} y \code{peekBoolean()}. El siguiente ejemplo lee la salida con \code{peekInt()},
que devuelve un entero de Scala\footnote{Para admitir enteros de anchura arbitraria, el valor
devuelto es un \codefoot{BigInt} de Scala en lugar de un \codefoot{Int}.}
que se usa en la sentencia \code{assert()}.
De forma parecida podemos leer la salida \code{equ} en un \code{Boolean} de Scala, que se usa
...
Este ejemplo es demasiado sencillo como para apreciar la ventaja de leer valores del DUT
a tipos de Scala. Sin embargo, en pruebas más complejas, por ejemplo un bucle que se repite hasta que
algún valor sea cierto, estas funciones resultan muy útiles.
...
En esta sección hemos descrito lo básico para escribir pruebas sencillas con Chisel.
Pero no pierdas de vista que dispones de toda la potencia de Scala para escribir las pruebas.
Eso incluye, por ejemplo, escribir en Scala un modelo de referencia de tu hardware contra el que
comprobar el DUT.
...
\myref{https://en.wikipedia.org/wiki/Unit_testing}{pruebas unitarias}, como es habitual en
el desarrollo de software. Un conjunto de pruebas unitarias puede servir además para las
\myref{https://en.wikipedia.org/wiki/Regression_testing}{pruebas de regresión}. Ahora bien,
para depurar diseños más complejos conviene poder examinar varias señales a la
vez. Una forma clásica de depurar diseños digitales es mostrar las señales en una
forma de onda (\emph{waveform}), donde se representan a lo largo del tiempo.
...
\noindent Los testers de Chisel pueden generar una forma de onda que incluya todos los registros y todas
las señales de IO. En los siguientes ejemplos mostramos testers de forma de onda para el
\code{DeviceUnderTest} del ejemplo anterior (la función AND de 2 bits).
Para generar una forma de onda de una prueba, pásale a la prueba la definición \code{writeVcd=1},
tal y como se ve en el siguiente comando de \code{sbt}:
...
Puedes ver la forma de onda con el visor gratuito
\myref{http://gtkwave.sourceforge.net/}{GTKWave} o con ModelSim.
Arranca GTKWave, selecciona \emph{File -- Open New Window} y ve a la
carpeta donde el tester de Chisel haya dejado el fichero \code{.vcd}. Por defecto, los ficheros generados
están en \code{test\_run\_dir}, dentro de una subcarpeta que toma su nombre de la prueba.
Ahí deberías encontrar \code{DeviceUnderTest.vcd}.
Puedes seleccionar las señales en la parte izquierda y arrastrarlas a la ventana principal.
Si quieres guardar una configuración de señales, puedes hacerlo con \emph{File -- Write Save File}
y cargarla más adelante con \emph{File -- Read Save File}.
...
La generación de formas de onda también puede activarse pasándole la anotación\\
\code{WriteVcdAnnotation} a la función \code{test()}.\footnote{Es una alternativa a
usar las opciones de la línea de comandos.}
Empezamos con un tester sencillo que hace \code{poke} de unos valores en las entradas y avanza
el reloj con \code{step}. No leemos ninguna salida ni la comparamos con \code{expect}.
En su lugar, inspeccionaremos la forma de onda generada en el fichero \code{.vcd}.
...
Enumerar de forma explícita todos los valores de entrada posibles no es escalable. Por eso vamos a utilizar
algo de código Scala para excitar el DUT. El siguiente tester enumera todos los valores posibles de
las dos señales de entrada de 2 bits.

\shortlist{code/test_bench_wavecnt.txt}

% \noindent and execute it with

\noindent y lo ejecutamos con

\begin{verbatim}
$ sbt "testOnly WaveformCounterTest"
\end{verbatim}

\subsection{Depuración con printf}
...
Otra forma de depurar es la ``depuración con printf''. El nombre de este método viene de
poner sentencias \code{printf} en el código C para imprimir las variables que interesan durante
la ejecución del programa. Esta depuración con printf también está disponible al probar
circuitos en Chisel. La impresión se produce en el flanco de subida del reloj.
Una sentencia \code{printf} puede insertarse en cualquier punto de la definición del módulo,
tal y como se ve en la versión del DUT con depuración con printf.
...
\noindent Al probar este módulo con el tester basado en un contador, que recorre todos los valores
posibles, obtenemos la siguiente salida, que confirma que la función AND es correcta:
...
El \code{printf} de Chisel admite
\myref{https://www.chisel-lang.org/chisel3/docs/explanations/printing.html}{formato al estilo de C y al
estilo de Scala}.
...
En este ejercicio volveremos al LED parpadeante de
\myref{https://github.com/schoeberl/chisel-examples}{chisel-examples}
y exploraremos las pruebas en Chisel.

\subsection{Un proyecto sencillo}
...
Primero, veamos en qué consiste un proyecto sencillo de Chisel. Explora los ficheros del ejemplo
\myref{https://github.com/schoeberl/chisel-examples/tree/master/hello-world}{Hello World}.
\code{Hello.scala} es el único fichero fuente de hardware.
Contiene la descripción hardware del LED parpadeante (\code{class Hello})
y una \code{App} que genera el código Verilog.
...
\noindent A continuación viene la descripción hardware, como se muestra en el código~\ref{lst:chisel:hello}.
Para generar la descripción en Verilog necesitamos una aplicación.
La única acción de esta aplicación es crear un nuevo objeto \code{Hello} y pasárselo
a la función \code{emitVerilog()}.

\shortlist{code/generate.txt}

% \noindent Run the generation of the example manually with

\noindent Lanza a mano la generación del ejemplo con
...
y explora con un editor el \code{Hello.v} generado. El código Verilog generado quizá no sea
muy legible, pero podemos averiguar algunos detalles. El fichero empieza con un módulo \code{Hello},
que tiene el mismo nombre que nuestro módulo de Chisel. Podemos identificar nuestro puerto del LED como
\code{output io\_led}. Los nombres de los pines son los nombres de Chisel precedidos de \code{io\_}.
Además del pin del LED, el módulo contiene también las señales de entrada \code{clock} y \code{reset},
que Chisel añade automáticamente.
...
También podemos identificar la definición de nuestros dos registros, \code{cntReg} y \code{blkReg}.
Al final de la definición del módulo encontraremos además el reset y la actualización de esos registros.
Fíjate en que Chisel genera un reset síncrono.

% For \code{sbt} to be able to fetch the correct Scala compiler and the Chisel library,
% we need a \code{build.sbt}:

Para que \code{sbt} pueda descargar el compilador de Scala y la biblioteca de Chisel correctos,
necesitamos un \code{build.sbt}:
\index{build.sbt}
...
\noindent Fíjate en que en este ejemplo fijamos un número de versión concreto de Chisel para no tener que
comprobar en cada ejecución si hay una versión nueva (lo que fallaría si no estamos conectados a Internet,
por ejemplo al hacer diseño hardware durante un vuelo). Además, hemos añadido el plugin del compilador
de Chisel3, necesario desde Chisel 3.5.
Cambia la configuración de \code{build.sbt} para usar la última versión de Chisel, modificando la
dependencia de la biblioteca a
...
\noindent y vuelve a lanzar la compilación con \code{sbt}. ¿Hay una versión más reciente de Chisel
disponible? ¿Se descarga automáticamente?
...
Por comodidad, el proyecto incluye también un \code{Makefile}.
Solo contiene el comando de \code{sbt}, así que no hace falta recordarlo y
podemos generar el código Verilog con:
...
Además de un fichero \code{README}, el proyecto de ejemplo contiene ficheros de proyecto
para distintas placas FPGA. Por ejemplo, en
\myref{https://github.com/schoeberl/chisel-examples/tree/master/hello-world/quartus/altde2-115}{quartus/altde2-115}
encontrarás los dos ficheros que definen un proyecto de Quartus para la placa DE2-115.
Las definiciones principales (ficheros fuente, dispositivo, asignación de pines) están en un fichero de
texto plano,
\myref{https://github.com/schoeberl/chisel-examples/blob/master/hello-world/quartus/altde2-115/hello.qsf}{hello.qsf}.
Explora el fichero y averigua qué pines están conectados a qué señales.
Si necesitas adaptar el proyecto a otra placa, es ahí donde se aplican los cambios.
Si tienes Quartus instalado, abre ese proyecto, compílalo con el botón verde \emph{Play}
y después configura la FPGA.
...
Ten en cuenta que el \emph{Hola Mundo} es un proyecto básico de Chisel.
Los proyectos más realistas organizan sus ficheros fuente en paquetes y contienen testers.
%The next chapter's exercise will explore such a project.

\subsection{Un ejercicio de pruebas}
...
En el ejercicio del capítulo anterior ampliaste el ejemplo del LED parpadeante con algunas entradas
para construir una puerta AND y un multiplexor, y ejecutaste ese hardware en una FPGA.
Ahora usaremos ese ejemplo y probaremos su funcionalidad con un tester de Chisel,
para automatizar las pruebas y no depender de una placa FPGA.
Toma tus diseños del capítulo anterior y añádeles un tester de Chisel que pruebe su funcionalidad.
Intenta enumerar todas las entradas posibles y comprobar la salida con \code{expect()}.
...
Probar dentro de Chisel puede acelerar la depuración de tu diseño.
Aun así, siempre es buena idea sintetizar el diseño para una FPGA y hacer pruebas
con ella. Ahí puedes contrastar con la realidad el tamaño de tu diseño (normalmente
en LUT y biestables) y su rendimiento en frecuencia máxima de reloj.
Como referencia, un procesador RISC segmentado de los que encontramos en un libro de texto puede 
ocupar unas 3000 LUT de 4 bits y funcionar a unos 100~MHz en una FPGA de gama baja (Intel Cyclone o
Xilinx Spartan).
